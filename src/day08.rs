...
impl Solver for Day {
    ...
    fn pt2(&self, inp: &str) -> String {
        decode(inp)
            ...
            .map(|vec|
                u32::from_str_radix(
                    &vec.iter().map(|d| d.to_string()).collect::<String>(),
                    10,
                ).unwrap()
            )
            .sum::<u32>()
...
fn resolve_digit(digits: &HashMap<u32, String>, input: &String) -> Option<u32> {
    fn missing(haystack: Option<&String>, needle: &String) -> Option<u32> {
        match haystack {
            None => None,
            Some(hay) => Some(needle.chars().filter(|c| hay.contains(*c)).collect::<Vec<char>>().len() as u32)
        }
    }
        ...
        for _ in 0..=1 {
            for i in 0..inputs.len() {
                ...
                if digit.is_some() {
                    ...
                    chars.sort();
                    digits.insert(digit.unwrap(), chars.into_iter().collect::<String>());
                }
            }
        }
